...
- Dependencies: AndroidX only (core-ktx, core-splashscreen, appcompat, lifecycle, material, swiperefreshlayout, constraintlayout)
- Splash screen: Android Splash Screen API (core-splashscreen:1.0.1) — branded dark background + app icon on launch, replaces old translucent launcher theme
- Edge-to-edge: system bar insets applied to root FrameLayout so all children (WebView, loading overlay, progress bar) render within safe areas
- Activities: LauncherActivity (router + splash), OnboardingActivity (welcome), AddServerActivity (add/edit), ServerListActivity (multi-server), MainActivity (WebView), ErrorActivity (connectivity)
...
- Icon rail navigation: Chat, Tasks, Code, Stats, Settings in left sidebar (hidden on mobile <768px)
- Mobile bottom navigation: 4-tab bar (Chat, Tasks, Code, Stats) visible on screens <768px, hidden when virtual keyboard open (<500px viewport height), uses MobileNav.svelte
- Mobile sidebar nav links: Tracker, Code, Stats, Settings, Voice Studio links at top of sidebar drawer on mobile
- Mobile header: settings gear icon visible on mobile (IconRail hidden), touch-friendly targets
- Responsive layout: flex-col on mobile, flex-row on desktop; h-dvh for proper mobile viewport handling
